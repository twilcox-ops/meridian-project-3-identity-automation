...
1. To read the certificate out of Key Vault, we use DefaultAzureCredential
   — locally this resolves to whoever ran `az login` (their own Azure AD
   identity must be granted a Key Vault access policy / RBAC role, e.g.
   "Key Vault Secrets User", to read `CERT_NAME`); in Azure (e.g. a
   Container Apps Job) it resolves to that resource's own system-assigned
   managed identity instead, with the exact same Key Vault role grant.
   Either way, no secret or credential for the Graph app is ever stored on
   disk or in env vars.
...
DefaultAzureCredential is what makes step 1 the same code path in both
places -- no environment-specific branching here, just which identity it
resolves to.
